...
server.tool("create_payment", "Create an Easypaisa payment.", createPaymentSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleCreatePayment(params) }] }));

server.tool("check_status", "Check Easypaisa payment status.", checkStatusSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleCheckStatus(params) }] }));

server.tool("create_ma_payment", "Create a mobile account payment.", createMaPaymentSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleCreateMaPayment(params) }] }));

server.tool("inquire_transaction", "Inquire about a transaction.", inquireTransactionSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleInquireTransaction(params) }] }));

server.tool("create_otc_payment", "Create an over-the-counter payment.", createOtcPaymentSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleCreateOtcPayment(params) }] }));

server.tool("generate_token", "Generate a payment token.", generateTokenSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleGenerateToken(params) }] }));

server.tool("refund_payment", "Refund an Easypaisa payment.", refundPaymentSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleRefundPayment(params) }] }));

server.tool("get_statement", "Get transaction statement.", getStatementSchema.shape,
